...
def check_es_service_software_version(cf, domain_name, region, slack_webhook, slack_channel):
    ...
    if service_software_update_available:
        ...
        slack_txt = (
            "*Service Software update avaliable for:* {0}. \n*Current Version:* {1} \n*New Version:* {2}".format(
            ...
            "attachments": [
                {
                    ...
                    "actions": [{"type": "button", "text": "Press Here to Update", "url": url_format}],
                }
            ],
        }
...
def check_es_version(es_client, domain_name, region, slack_webhook, slack_channel):
    ...
    if len(compatible_target_versions) == 0:
        ...
        print(
            "Latest version of elastic Search that is compatible with Domain {0} is: {1}".format(
                domain_name, latest_version
            )
        )
        ...
        slack_message = {
            ...
            "attachments": [
                {
                    "fallback": "Update the domain through AWS elastic Search Service Console",
                    "actions": [{"type": "button", "text": "AWS Console", "url": url_format}],
                }
            ],
        }
        data = json.dumps(slack_message).encode("utf-8")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--domain-name-list",
        help="The domain name in elastic search that requires update ",
        dest="domain_name_list",
        required=False,
    )
    parser.add_argument("--region", help="The AWS region the stack is in", dest="region", required=True)
    parser.add_argument(
        "--profile", help="The name of an aws cli profile to use.", dest="profile", default=None, required=True
    )
